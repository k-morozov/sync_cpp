...
template<typename T>
class BufferedMPMCExecutor final {
public:
	explicit BufferedMPMCExecutor(const size_t senders_count,
	...
	{}
	~BufferedMPMCExecutor() { chan_.Close(); }
...
private:
	sync_cpp::buffered_mpmc_channel<T> chan_;
...
//		ASSERT_TRUE(all_senders == all_receivers);
	...
	void SendData(std::vector<T> & values, sync_cpp::buffered_mpmc_input_channel<T> & chan) {
		while (true) {
	...
	void RecvData(std::vector<T> & values, sync_cpp::buffered_mpmc_output_channel<T> & chan) {
		while (true) {
